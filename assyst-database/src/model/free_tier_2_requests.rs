...
impl FreeTier2Requests {
    ...
    pub async fn change_free_tier_2_requests(
        &self,
    ...
    pub async fn get_user_free_tier_2_requests(
        handler: &DatabaseHandler,
        user_id: u64,
    ) -> anyhow::Result<FreeTier2Requests> {
        let fetch_query = "select * from free_tier1_requests where user_id = $1";

        match sqlx::query_as::<_, FreeTier2Requests>(fetch_query)
            .bind(user_id as i64)
            .fetch_one(&handler.pool)
            .await
        {
            Ok(x) => Ok(x),
            Err(sqlx::Error::RowNotFound) => Ok(FreeTier2Requests {
                user_id: user_id as i64,
                count: 0,
            }),
            Err(e) => Err(e.into()),
