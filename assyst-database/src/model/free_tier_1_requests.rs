...
impl FreeTier1Requests {
    pub async fn change_free_tier_1_requests(
    ...
    pub async fn get_user_free_tier_1_requests(
        handler: &DatabaseHandler,
        user_id: u64,
    ) -> anyhow::Result<FreeTier1Requests> {
        let fetch_query = "select * from free_tier1_requests where user_id = $1";

        match sqlx::query_as::<_, FreeTier1Requests>(fetch_query)
            .bind(user_id as i64)
            .fetch_one(&handler.pool)
            .await
        {
            Ok(x) => Ok(x),
            Err(sqlx::Error::RowNotFound) => Ok(FreeTier1Requests {
