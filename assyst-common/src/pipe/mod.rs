...
/// Pipe is a utility class that wraps a [UnixStream], providing helper functions for easy reading and writing of
/// serde-Serializable types via Bincode.
pub struct Pipe {
...
impl Pipe {
    ...
    /// 
    /// This function will return an Err if the stream is prematurely closed, or if Bincode is not able
    /// to deserialize the data to the specified type.
    ...
    /// 
    /// This function will return an Err if the stream is prematurely closed, or if Bincode is not able
    /// to serialize the data to the specified type.
