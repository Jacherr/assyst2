...
use assyst_common::util::process::{exec_sync, get_processes_cpu_usage, get_processes_mem_usage, get_processes_uptimes};
...
pub async fn stats(ctxt: CommandCtxt<'_>, option: Option<Word>) -> anyhow::Result<()> {
    fn get_process_stats() -> String {
        ...
        let mem_usages_fmt = mem_usages.iter().map(|x| (x.0.fg_cyan(), human_bytes(x.1 as f64))).collect::<Vec<_>>();

        let cpu_usages = get_processes_cpu_usage();
        let cpu_usages_fmt = cpu_usages.iter().map(|x| (x.0.fg_cyan(), format!("{:.2?}", x.1) + "%")).collect::<Vec<_>>();
        ...
        for i in mem_usages_fmt {
            ...
            {
                combined_usages.push((cpu.0.clone(), format!("Memory: {}, CPU: {}, Uptime: {}", i.1, cpu.1, uptime.1)));
            }
        }
    ...
    async fn get_session_stats(ctxt: &CommandCtxt<'_>) -> anyhow::Result<String> {
        ...
        let SessionStartLimit { total, remaining, max_concurrency, .. } = gateway_bot.session_start_limit;

        let table = key_value(&[("Total".fg_cyan(), total.to_string()), ("Remaining".fg_cyan(), remaining.to_string()), ("Max Concurrency".fg_cyan(), max_concurrency.to_string())]);
    ...
    async fn get_storage_stats(ctxt: &CommandCtxt<'_>) -> anyhow::Result<String> {
        ...
        let filer_stats = filer_stats(ctxt.assyst().clone()).await.unwrap_or(FilerStats { count: 0, size_bytes: 0 });
    ...
    async fn get_general_stats(ctxt: &CommandCtxt<'_>) -> String {
        ...
        let commit = exec_sync("git rev-parse HEAD").map(|x| x.stdout[..8].to_owned()).unwrap_or("Unknown".to_string());

        let stats_table = key_value(&[
            ("Guilds".fg_cyan(), ctxt.assyst().metrics_handler.guilds.get().to_string()),
            ("Shards".fg_cyan(), ctxt.assyst().shard_count.to_string()),
            ("Events".fg_cyan(), format!("{events_rate}/sec ({events_total} since restart)")),
            ("Commands Executed".fg_cyan(), commands_rate + "/min"),
            ("Commit Hash".fg_cyan(), commit),
            ("Flux Version".fg_cyan(), ctxt.assyst().flux_handler.get_version().await.unwrap_or("Unknown".to_owned())),
        ]);
        ...
        && x.to_lowercase() == "all"
    {
        ...
        let full_output = format!("**General**\n{stats_table}\n**Processes**\n{usages_table}\n**Storage and Caches**\n{storage_table}\n**Sessions**\n{session_table}");
